...
object Compare extends App {
  ...
  Connection.query("select * from table").catchAll { _ =>
    ZIO.succeed(Nil)
  }
  ...
  def loadRemoteData: ZIO[Clock with Logging, Nothing, String] = {
    Logging.info("open remote connection").bracket(_ => Logging.info("close remote connection")) { _ =>
      ZIO
        .foreach((1 to 10).toList) { i =>
  ...
  val race = loadRemoteData race loadCacheData.tap(data => Logging.info("返回: " + data))

  type User  = String
  type Image = Array[Byte]
  val image = Array.empty[Byte]

  def findUser(name: String): IO[Option[Nothing], User]      = ???
  def retrieveAvatar(user: User): IO[Option[Nothing], Image] = ???
  ...
  def upload(user: User, image: Image): IO[String, URI] = ???
