...
object mtf {
  ...
  val io: IO[Result, Result] = for {
    status <- ZIO.fromOption(parseStatus(statusStr)).orElseFail(BadRequest(s"状态($statusStr)不合法"))

    _ <- checkPermission(resourceType, request.member).mapError(msg => Unauthorized(msg))

    resource <- findResource(resourceId).mapError(err => InternalError(err.getMessage)).someOrFail(NotFound(s"资源 $resourceId 不存在"))

    result <- process(resource, status).mapError(msg => BadRequest(msg))
  } yield Ok(result)
}
